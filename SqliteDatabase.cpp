...
bool SqliteDatabase::submitGameStatsToDB(std::string playerName, GameData& data)
{
    ...
    const char* sql =
        ...
        "   AVG_ANSWER_TIME = ((STATISTICS.AVG_ANSWER_TIME * STATISTICS.NUM_TOTAL_ANSWERS) + ?) / (STATISTICS.NUM_TOTAL_ANSWERS + ?), " // ?5, ?6
    ...
    int currentQuestionsCount = data.correctAnswerCount + data.wrongAnswerCount;
    double currentTotalTime = data.averageAnswerTime * currentQuestionsCount;
    ...
    sqlite3_bind_int(stmt, 4, currentQuestionsCount);

    sqlite3_bind_double(stmt, 5, currentTotalTime);

    sqlite3_bind_int(stmt, 6, currentQuestionsCount);

    sqlite3_bind_int(stmt, 7, data.correctAnswerCount);

    sqlite3_bind_int(stmt, 8, currentQuestionsCount);
